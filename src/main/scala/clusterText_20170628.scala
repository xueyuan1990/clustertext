...
object clusterText_20170628 {
  ...
  def training(threshold: Double, word_size: Int, articles: RDD[(Array[String], Array[(String, Double)], Array[Double])], stat_date_flag: Boolean): RDD[(Array[String], Array[(String, Double)], Array[Double])] = {
    ...
    def find_cluster(iterator: Iterator[(Array[String], Array[(String, Double)], Array[Double])]): Iterator[(Array[String], Array[(String, Double)], Array[Double])] = {
      ...
      if (id_wordweight_vector.size >= 1) {
        ...
        for (i <- 1 until id_wordweight_vector.size) {
          val (c, count_num) = find_cluster_rest(clusters, id_wordweight_vector(i), threshold, word_size, stat_date_flag)
          clusters = c
