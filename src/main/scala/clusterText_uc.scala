...
object clusterText_uc {
  ...
  def main(args: Array[String]): Unit = {
    ...
    val error_rate2 = 1 - result.filter(r => (r._1.split(",")(0)).equals(r._2.split(",")(0))).count() / result.count().toDouble
  ...
  def load_data(sql: String): RDD[(Long, Array[String], String)] = {
    ...
    val id_word_cate = df.map(r => (r.getLong(0), r.getString(1).split(",").take(3), r.getString(2))).map(r => {
      var array = new ArrayBuffer[String]()
      for (w <- r._2) {
        array += w.split(":")(0).trim.replace(" ", "_")
      }
