...
//  val tfidfTable = "algo.lj_article_word_tfidf"
  ...
  val articlWordTable = "algo.lc_article_word"
  val tfidfTable = "algo.lc_article_word_tfidf"
...
//    val selectSrc =
//      if(test) s"select id,title,content from $srcTable where 1494022103132001 < id and id < 1494037605132008"
//      else s"select id,title,content from $srcTable "

    val selectSrc =
      if(test) s"select fid,ftitle,cast(unbase64(fcontent) as string) from $srcTable where 1494022103132001 < fid and fid < 1494037605132008"
      else s"select fid,ftitle,cast(unbase64(fcontent) as string) from $srcTable where fresource_type=2  and fid is not null and fkeywords is not null and fkeywords != ''"
