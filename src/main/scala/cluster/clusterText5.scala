...
/**
  * Created by xueyuan on 2017/5/9.收敛
  */
object clusterText5 {
  ...
  def load_data(sql: String): RDD[(Array[String], Map[String, Double], Array[Double])] = {
  ...
  def training(threshold: Double, word_size: Int, articles: RDD[(Array[String], Map[String, Double], Array[Double])]): RDD[(Array[String], Map[String, Double], Array[Double])] = {
  ...
  def get_word_weight(word_weigth_1: Map[String, Double], word_weigth_2: Map[String, Double]): Map[String, Double] = {
    ...
    var word_weight_union: Map[String, Double] = new mutable.HashMap[String, Double]()
    for (word <- words) {
      val weight = word_weigth_1.getOrElse(word, 0.0) + word_weigth_2.getOrElse(word, 0.0)
      word_weight_union += (word -> weight)
    }
    word_weight_union = word_weight_union.toList.sortWith(_._2 > _._2).toMap
    word_weight_union
  }
}
