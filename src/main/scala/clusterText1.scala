...
object clusterText1 {
  ...
  def main(args: Array[String]): Unit = {
    ...
    table_name = table_name + threshold
  ...
  def training(): RDD[(Array[String], Map[String, Double], Array[Double])] = {
    ...
    val id_wordweight_vector_rdd = sc.parallelize(articals)
    id_wordweight_vector_rdd.repartition(partition_num)
    println("***********************partition_num = " + partition_num + "*****************************")
    var clusters_temp = id_wordweight_vector_rdd.mapPartitions(find_cluster)
  ...
  //返回cluster
  def find_cluster(iterator: Iterator[(Array[String], Map[String, Double], Array[Double])]): Iterator[(Array[String], Map[String, Double], Array[Double])] = {
    var id_wordweight_vector: ArrayBuffer[(Array[String], Map[String, Double], Array[Double])] = ArrayBuffer[(Array[String], Map[String, Double], Array[Double])]()
    var clusters: ArrayBuffer[(Array[String], Map[String, Double], Array[Double])] = new ArrayBuffer[(Array[String], Map[String, Double], Array[Double])]()
    while (iterator.hasNext) {
      id_wordweight_vector += iterator.next()
    }
    ...
    if (id_wordweight_vector.size == 1) {
      ...
      if (sim > threshold) {
        ...
        val union = (id_union, word_weight_union, vector_union)
        clusters += union
      } else {
        clusters += data0
        clusters += data1
      ...
      for (i <- 2 until id_wordweight_vector.size) {
        clusters = find_cluster_rest(clusters, id_wordweight_vector(i))
      }
    }
  ...
  def find_cluster_rest(clusters: ArrayBuffer[(Array[String], Map[String, Double], Array[Double])], article: (Array[String], Map[String, Double], Array[Double])): ArrayBuffer[(Array[String], Map[String, Double], Array[Double])] = {
